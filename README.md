...
- `get_system_health` now scrubs secret-keyed values and URL-embedded credentials from the per-integration health data before returning it, so a diagnostics-capable token cannot pull an integration's API key or connection-string password into the agent's context. Documented that `get_yaml_config` returns `configuration.yaml` verbatim (inline secrets included; prefer `!secret` references), and that a saved dashboard layout carries the same indirect-control caveat as authored automations.
